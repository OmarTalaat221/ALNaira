...
const CourseListTable = ({ Courses, showHideCourse, getCourses }) => {
  ...
  const handleupdatecourse = () => {
    ...
    axios.post("https://camp-coding.tech/dr_elmatary/admin/image_uplouder.php", formdata)
      .then((res) => {
        ...
        axios.post(base_url+"/admin/courses/edit_course.php", JSON.stringify(data_send))
          .then((res) => {
            console.log(res)
            ...
            <UncontrolledDropdown className="DropVidUn">
              ...
              <DropdownMenu className="dropdown-menu-end">
                ...
                <DropdownItem>
                  <span onClick={
                    () => {
                      navigate("/coursesqueries", { state: { course_data: cell.cell.row.original } })
                      // setshowcourseedit(true);
                      // setrowdata(cell.cell.row.original);
                      // console.log(cell.cell.row.original)
                    }
                  }>
                    <button className='btn btn-warning' style={{ width: "100%" }}>Student Questions</button>
                  </span>
                </DropdownItem>
  ...
  const duplicateCourse = (e) => {
    ...
    axios.post(base_url+"/admin/courses/make_copy_from_course.php", JSON.stringify(data_send))
      .then((res) => {
        console.log(res)
        if(res.message=='Session Expired'){
          // toast.error();
