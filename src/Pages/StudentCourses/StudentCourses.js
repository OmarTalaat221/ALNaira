...
const StudentCourses = () => {
  ...
  const getStudentSub = async () => {
    const subStudent = await axios.post("https://elmatary.com/El_Matary_Platform/platform/admin/students/select_student_subscription.php", JSON.stringify({ student_id: location?.state?.unitData?.student_id }));
    setSubs(subStudent?.message);
    console.log(location?.state?.unitData?.student_id);
  }
  const cancelSub = async (e) => {
    const subStudent = await axios.post("https://elmatary.com/El_Matary_Platform/platform/admin/subscription/cancel_student_sub.php", JSON.stringify({ student_id: location?.state?.unitData?.student_id, cancel_reason: e?.currentTarget?.cancel_notes?.value }));
