...
const Universities = () => {
  document.title = "Companies | ALNaierh  ";
  ...
  const handleaddnewuni = async () => {
    if (university_name == "" || university_name == null) {
      toast.warn("Enter Company Name");
      return;
    }

    if (!univData?.imageFile) {
      toast.warn("Enter Company Image");
      return;
    }
    let imageUrl = null;
    if (univData?.imageFile) {
      imageUrl = await uploadFile(univData?.imageFile);
    }
    ...
    axios
      .post(
        base_url + "/admin/universities/insert_university.php",
        JSON.stringify(data_send)
      )
      .then((res) => {
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid={true}>
          <Breadcrumbs title="Companies" breadcrumbItem="Company List" />
          <Row>
            <Col lg={12}>
              <Card>
                <CardBody>
