...
const DoNotMarry = () => {
  ...
  const getStudents = async (e) => {
    ...
    const student = await axios.post(
      "https://camp-coding.online/Teacher_App_2025/elnaira_jor/admin/students/select_students.php",
      data_send
    );
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        ...
        <Modal title="Assign Student" isOpen={showAssign}>
          <form
            action="#"
            style={{
              padding: "15px",
              ...
              >
                {student && student.length
                  ? student.map((item, index) => {
                      return (
                        <MenuItem value={item.student_id} key={index}>
                          {item.student_name} - {item.university_name} -{" "}
                          {item.grade_title}
                        </MenuItem>
                      );
                    })
