...
function Subscription() {
  ...
  const getSubs = async () => {
    setPageLoading(true)
    const subs = await axios.get("https://elmatary.com/El_Matary_Platform/platform/admin/subscription/select_student_sub.php")
    .finally(()=>{
  ...
  const cancelSub = async (e) => {
    ...
    const subStudent = await axios.post("https://elmatary.com/El_Matary_Platform/platform/admin/subscription/cancel_student_sub.php", JSON.stringify({ cancel_reason: e?.currentTarget?.cancel_notes?.value, subscription_id: item?.subscription_id }));
