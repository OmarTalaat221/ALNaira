...
const Videos = () => {
  ...
  const getVideos = async () => {
    ...
    const videos = await axios.post(base_url+"/admin/videos/select_videos.php",JSON.stringify(data_send));
    console.log(videos)
    setVideos([...videos])
  ...
  const editVideo = async (e) => {
    ...
    const data_send = {
      ...
      "youtube_id": e.currentTarget.youtube_id.value ? e.currentTarget.youtube_id.value : item.youtube_id,
    ...
    const assign = await axios.post(base_url+"/admin/videos/edit_source_video.php", data_send);
    console.log(assign)
    if(assign.message=='Session Expired'){
      localStorage.removeItem('elmatary_admin');
      navigate('/login',{replace:true})
          ...
          <>
            <button className="btn btn-warning" onClick={
              () => {
                ...
                navigate('/videoquestions',{state:{course_data:cell.cell.row.original}});
              }
            }>Video Questions</button>
